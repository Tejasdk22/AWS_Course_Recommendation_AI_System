"""
Bedrock Integrated Orchestrator
Properly integrates with Bedrock AgentCore agents
"""
...
def handler(event, context):
    ...
    try:
        ...
        # Step 1: Job Market Agent - Get market data
        logger.info('Step 1: Getting job market data from Bedrock...')
        job_market_data = invoke_bedrock_agent(
            agent_id='UYMIN4MNDB',  # JobMarketAgent
            query=f"Analyze job market for {career_goal} roles. Include salary ranges, required skills, and market trends for {major} students.",
            session_id=f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        
        # Step 2: Course Catalog Agent - Get relevant courses
        logger.info('Step 2: Getting course catalog data from Bedrock...')
        ...
        logger.info('Step 3: Matching courses to career from Bedrock...')
        career_matching_data = invoke_bedrock_agent(
            agent_id='VXP3OUEGFI',  # CareerMatchingAgent
            query=f"Match {major} courses to {career_goal} career path. Analyze skill gaps and recommend courses for {student_type} students.",
            session_id=f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        
        # Step 4: Project Advisor Agent - Get project recommendations
        logger.info('Step 4: Getting project recommendations from Bedrock...')
        project_advisor_data = invoke_bedrock_agent(
            agent_id='QRPN5E3P2A',  # ProjectAdvisorAgent
            query=f"Recommend hands-on projects for {major} {student_type} students pursuing {career_goal}. Include Kaggle, GitHub, and portfolio projects.",
            session_id=f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        response = {
            ...
            'specific_career_analysis': {
                ...
                'market_analysis': {
                    'job_market_summary': {
                        ...
                        'market_insights': job_market_data.get('response', 'Market data analysis from Bedrock agents')
                    },
            ...
            'agent_coordination': {
                ...
                'bedrock_agents_used': ['JobMarketAgent', 'CourseCatalogAgent', 'CareerMatchingAgent', 'ProjectAdvisorAgent']
            }
        }
...
def invoke_bedrock_agent(agent_id: str, query: str, session_id: str) -> Dict[str, Any]:
    ...
    try:
        logger.info(f'Invoking Bedrock agent {agent_id} with query: {query[:100]}...')
        ...
        logger.info(f'Bedrock agent {agent_id} response length: {len(result)}')
...
def generate_course_recommendations(career_goal: str, major: str, student_type: str, 
    ...
    return {
        ...
        'bedrock_insights': {
            'job_market_analysis': job_market_data.get('response', ''),
            'course_catalog_analysis': course_catalog_data.get('response', ''),
            'career_matching_analysis': career_matching_data.get('response', ''),
            'project_recommendations': project_advisor_data.get('response', '')
        }
    }
